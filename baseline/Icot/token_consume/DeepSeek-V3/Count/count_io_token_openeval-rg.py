...
def process_fewshot_task(
    ...
    xcot_messages.append({"role": "user", "content": code_user_content})
...
def process_task(
    ...
    xcot_messages = [
        {"role": "system", "content": config["messages"]["xcot_system"]}
    ]
    ...
    for xcot_entry in xcot_entries:
        ...
        code_user_content = config["messages"]["code_user"].format(
            combined_xcot_prompt=combined_xcot_prompt
        )
...
def main():
    ...
    for task_entry in dataset_entries:
        ...
        routing_label = task_entry["routing_label"]        
        task_codes = [c for c in code_entries if c["task_id"] == task_id]
        if routing_label == "Easy":
            result = process_fewshot_task(task_id, prompt, task_codes, token_counter, config)
        else: 
            task_xcots = [x for x in xcot_entries if x["task_id"] == task_id]
            result = process_task(task_id, prompt, task_xcots, task_codes, token_counter, config)
